...
echo "SQSH:"
sqsh -S tnh_mss -U "${user}" -C "select FirstName, LastName, BirthDate, cast(PersonUid as varchar(max)) as PersonUid from prm.dbo.person where (FirstName = '${px_FirstName}' and LastName = '${px_LastName}') or (FirstName = '${rx_FirstName}' and LastName = '${rx_LastName}')" -m pretty
...
echo "SQSH:"
sqsh -S tnh_mss -U "${user}" -C "select cast(VisitUid as varchar(max)), cast(year(VisitDate) as varchar(max)) + '-' + cast(month(VisitDate) as varchar(max)) + '-' + cast(day(VisitDate) as varchar(max)) + '_' + cast(cast(VisitDate as time) as varchar(max)) from prm.dbo.visit where PatientUid = '${PatientUid}' and ProviderUid = '${DoctorUid}'" -L bcp_colsep='|' -L bcp_rowsep='' -m bcp > /tmp/aprima_VisitNotes_export.csv
...
echo "SQSH:"
sqsh -S tnh_mss -U "${user}" -L bcp_colsep='' -L bcp_rowsep='' -i /tmp/aprima_VisitNotes_export.sqsh
