...
echo "SQSH:"
sqsh -S "${server}" -U "${user}" -C "select FirstName, LastName, BirthDate, cast(PersonUid as varchar(max)) as PersonUid from prm.dbo.person where (FirstName = '${px_FirstName}' and LastName = '${px_LastName}') or (FirstName = '${rx_FirstName}' and LastName = '${rx_LastName}')" -m pretty
...
echo "SQSH:"
sqsh -S "${server}" -U "${user}" -L bcp_colsep='' -L bcp_rowsep='' -i /tmp/aprima_VisitNotes_export.sqsh
